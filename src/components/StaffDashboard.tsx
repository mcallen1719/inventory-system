...
interface RecentActivitiesReportProps {
  staffName: string;
  currency: string;
  onRefresh: () => void;
}

function RecentActivitiesReport({ staffName, currency, onRefresh }: RecentActivitiesReportProps) {
  const recentActivities = DBStore.getStaffRecentActivities(staffName, 24);
  ...
  const handleReport = (id: string, ref: string, type: string) => {
    ...
    DBStore.reportActivity(type.toLowerCase().replace(" sales report", "sales_report").replace("gpo", "gpo").replace("job", "job").replace("misc", "misc") as any, id, ref, staffName, reportReason.trim());
    setReportingId(null);
    setReportReason("");
    onRefresh();
          ...
          <tbody className="divide-y divide-white/5">
            {recentActivities.length === 0 ? (
              ...
              <tr key={a.id} className="hover:bg-white/5 transition">
                ...
                <td className="px-3 py-2 text-right font-black text-gray-900 dark:text-white">{currency} {(a.amount || 0).toFixed(2)}</td>
                <td className="px-3 py-2 text-gray-500 dark:text-zinc-500">{a.timestamp ? new Date(a.timestamp).toLocaleString() : "—"}</td>
    ...
    <div className="space-y-6">
      ...
      {activeTab === "overview" && (
        <div className="space-y-6">
          ...
          {/* ==================================================== */}
          <RecentActivitiesReport staffName={activeUserName} currency={currency} onRefresh={onRefreshGlobalState} />
                ...
                {miscs.map((misc) => (
                  <tr key={misc.id} className="hover:bg-white/5 transition-colors duration-150">
                    ...
                    <td className="py-3.5 text-right font-black text-rose-500">
                      {currency} {(misc.amount || 0).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
